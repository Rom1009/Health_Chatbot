...
def main():
    ...
    llm = ChatGroq(
        temperature=0, 
        groq_api_key = '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', 
        model_name=model
    )
    ...
    if user_question:
        ...
        crew = Crew(
            agents=[Problem_Definition_Agent, Data_Assessment_Agent, Model_Recommendation_Agent], #, Summarization_Agent, Starter_Code_Generator_Agent],
            tasks=[task_define_problem, task_assess_data, task_recommend_model], #, task_summarize],task_generate_code
            verbose=2
        )
